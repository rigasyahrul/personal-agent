...
<aside
  class="workspace-panel session-layout__workspace panel form-stack"
  aria-label="Workspace"
>
  <h2 class="text-base font-semibold" style="margin:0">Workspace files</h2>
  {#if loading}
  ...
  {:else}
    ...
    <div class="workspace-tree max-h-56 space-y-1 overflow-auto text-sm">
      {#each entries as entry (entry.path)}
        <button
          type="button"
          class="tree-item {changed.has(entry.path) ? 'bg-amber-50' : ''} {entry.kind === 'directory' ? 'text-slate-500' : ''}"
