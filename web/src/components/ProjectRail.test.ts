...
describe('ProjectRail', () => {
  ...
  it('defaults to Config with SOUL SYSTEM AGENTS and no fake unsaved field', async () => {
    ...
    expect(await screen.findByRole('textbox', { name: 'SOUL' })).toHaveValue('Ship it.\n')
    expect(screen.queryByRole('textbox', { name: 'Instructions (system)' })).toBeNull()
    expect(screen.queryByText(/not saved yet/i)).toBeNull()
    expect(screen.queryByTestId('memory-lessons-preview')).toBeNull()
    expect(screen.queryByText('No lessons yet.')).toBeNull()
    expect(document.querySelector('.instruction-editor--rail')).toBeTruthy()
    expect(document.querySelector('.rail-panel--config .panel')).toBeNull()
  })

  it('saves the selected project instruction from Config', async () => {
    render(ProjectRail, { props: { projectId: 'p1' } })
