...
export function operationBadge(operation,onRetryCards,{retryDisabled=false}={}){
  ...
  if(operation.retry_cards===true&&operation.pending_id&&typeof onRetryCards==='function'){
    const button=document.createElement('button');button.type='button';button.textContent='Retry cards';button.disabled=retryDisabled;button.addEventListener('click',()=>{Promise.resolve(onRetryCards(operation,button)).catch(()=>{})});el.append(' ',button)
  }
  return el
}
