...
- **Named URL: open it and do the action. Never pretend you used the UI.** If the user gives a link or says “open the browser” / “type `@`”: Chrome CDP on **that URL**, interact, screenshot. `lsof`, curl of HTML/`/src/*.svelte`, “the JS is served”, or git-merged ≠ used the app. Do not answer as if you typed in the composer. User correction 2026-08-27. → craft skill + `docs/memory/20260827-1631-never-claim-ui-without-opening-named-url.md`
- **Composer `@` is not a hidden grant.** Hub start **and** open-session Reply both show file suggestions on `@`. Do not gate that list on `workspace_files` / `workspaceEnabled` — there is no UI for it. Live sessions often store `session_files: true` only. List GET may be ungated; if tree 403s, fall back to project notes. User loved this 2026-08-27. → `docs/memory/20260827-1810-composer-at-mentions-no-hidden-grant.md`
