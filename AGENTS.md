...
- **Every worker task MUST pass consulting-grok-review before merge.** After each implementer (Grok `-x` worker) finishes a plan task/phase: package the task diff (`review-package` or `git diff BASE..HEAD`), dispatch a **new** `amp -m grok45 --no-archive-after-execute -x` thread with `consulting-grok-review` / `reviewer-prompt` (never Task/OpenAI/oracle/self-review). Fix Critical+Important (or re-review the fix range) before FF-merge into the feature branch. No "tests green = reviewed." Whole-branch review still required before ship. → `.agents/skills/consulting-grok-review/`
