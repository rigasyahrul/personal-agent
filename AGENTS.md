...
- **UI code without browser = not done (HARD).** Whenever you change markup/CSS/layout/spacing/visible copy: **open the real app in a browser** (Chrome DevTools MCP, puppeteer, or equivalent), drive the changed surface, and report **URL + what you saw** (screenshot/metrics OK). **Forbidden:** ship/claim done from CSS reading, unit tests, or curl alone. **Forbidden:** “looks fine in code.” If the app is down: start `make docker-dev` (preferred local UI loop) or say **blocked** — never treat blocked as passed. User correction 2026-08-21: agents must not blindly code UI without UX verification. → craft skill + `docs/memory/20260821-1700-ui-changes-require-browser-vibe-pass.md`
