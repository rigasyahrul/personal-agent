...
- **Browser:** `agent-browser` / puppeteer / Chrome DevTools. A11y snapshot is enough for craft gates; copy durable shots to `.amp/in/artifacts/`.
- **Worktree vibe-pass:** do **not** use laptop `make docker-dev` `:8080` (compose mounts `..:/src` = laptop `main`; host `:8080` may be SSH). Serve **this** checkout (`PA_ADDR=:1808x`, `PA_MODELS`, `web/dist` or Vite).
