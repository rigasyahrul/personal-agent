...
- **Project hub (Claude stack):** startSession is not create∧send atomic — after create, always list+open session even if first send fails. Sessions list errors soft-fail (composer stays; Retry in list). Rail Files: project notes → `getProjectNote` (`source: 'project-note'` + `noteId`); workspace paths → `workspaceFile`. Default hub grant is `workspace_files: false`. **Config rail** = persisted SOUL / SYSTEM / AGENTS (`InstructionEditor` `variant="rail"`). Never a second instruction card on the hub canvas; never the fake unsaved “Instructions (system)” textarea. → `AGENTS.md` hub/rail bullets + `ProjectHubPage` / `ProjectRail` tests
...
- **Browser (required for UI):** `.chrome.mcp.json` → `chrome-devtools-mcp` on `http://localhost:9222`. If `:9222` is down, start a **separate** Chrome (`--remote-debugging-port=9222 --user-data-dir=/tmp/chrome-debug-pa`). Drive via CDP (navigate, a11y/DOM snapshot, `getBoundingClientRect`). **Do not** desktop screen-record (`screencapture` of the user’s machine). Auth: seed `pa_session` if the debug profile is logged out. Screenshots of the **page** (CDP) → `.amp/in/artifacts/` when durable.
