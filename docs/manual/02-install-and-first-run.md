...
- Optional: **Docker Compose** for packaged deploy  
...
1. If no owner exists, you see **Set up your owner account**.  
2. Enter **Bootstrap token** (same as `BOOTSTRAP_TOKEN`) and a **password (12+ characters)**.  
3. Continue → **Sign in** with that password.  
4. Nav appears: Home / Review / Settings. Header should show storage **ready**.
...
Without models, the Sessions page tells you to open Settings / configure models (env-driven list). Live replies also need `OPENAI_API_KEY` (and optional `OPENAI_BASE_URL`).
